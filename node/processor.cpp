...
NodeProcessor::DataStatus::Enum NodeProcessor::OnBlock(const NodeDB::StateID& sid, const Blob& bbP, const Blob& bbE, const PeerID& peer)
{
	...
	if (nSize > Rules::get().MaxBodySizeD)
	{
...
Difficulty NodeProcessor::get_NextDifficulty()
{
	...
	if ( (m_Cursor.m_Full.m_Height + 1 >= r.pForks[1].m_Height) && (m_Cursor.m_Full.m_Height - Rules::HeightGenesis < r.pForks[1].m_Height + r.DA.WindowWork + 1) )
		{
			LOG_INFO() << "1diff fork start. " << "fullmheight:" << m_Cursor.m_Full.m_Height << "sid.m_row:" << m_Cursor.m_Sid.m_Row << "sid.m_height:" << m_Cursor.m_Sid.m_Height;
			return r.DA.Difficulty0; // 1st block difficulty 0
		}

  LOG_INFO() << "test. " << "fullmheight:" << m_Cursor.m_Full.m_Height << "sid.m_row:" << m_Cursor.m_Sid.m_Row << "sid.m_height:" << m_Cursor.m_Sid.m_Height;
	...
	if (m_Cursor.m_Full.m_Height - Rules::HeightGenesis >= r.DA.WindowWork)
	{
		...
		LOG_INFO() << "diff standart. " << "fullmheight:" << m_Cursor.m_Full.m_Height << "sid.m_row:" << m_Cursor.m_Sid.m_Row << "sid.m_height:" << m_Cursor.m_Sid.m_Height;
	}
	else
	{
		LOG_INFO() << "diff standart < 120. " << "fullmheight:" << m_Cursor.m_Full.m_Height << "sid.m_row:" << m_Cursor.m_Sid.m_Row << "sid.m_height:" << m_Cursor.m_Sid.m_Height;
	...
	assert(r.DA.WindowWork > r.DA.WindowMedian1); // when getting median - the target height can be shifted by some value, ensure it's smaller than the window
		...
		if (m_Cursor.m_Full.m_Height - Rules::HeightGenesis >= r.pForks[1].m_Height + r.DA.WindowWork + 1)
		{
			dtTrg_s = r.DA.DTarget_s * dh;
		} else {
			dtTrg_s = r.DA.Target_s * dh;
		}
...
size_t NodeProcessor::GenerateNewBlockInternal(BlockContext& bc)
{
	...
	const size_t nSizeMax = Rules::get().MaxBodySizeD;
...
bool NodeProcessor::GenerateNewBlock(BlockContext& bc)
{
	...
	return nSize <= Rules::get().MaxBodySizeD;
}
