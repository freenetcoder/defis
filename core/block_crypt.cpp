...
#define COMPARE_TYPE(var, fnNext) \
 ...
 \
	...
	Prehistoric = {
		...
		0x00, 0x22, 0xfa, 0x38, 0x04, 0xb5, 0x8a, 0xdb,
		0x90, 0x05, 0x11, 0xdd, 0x1a, 0x8a, 0x75, 0xb5,
		0xd7, 0x93, 0xa7, 0xd3, 0x72, 0xac, 0x93, 0xb2,

	};

	ZeroObject(pForks);

	pForks[1].m_Height = 444400;
	...
	Amount Rules::get_EmissionEx(Height h, Height& hEnd, Amount base) const
	{
		...
		if (h < pForks[1].m_Height - 1)
		{
			hEnd = pForks[1].m_Height;
			return base;
		}

		if ( (h >= pForks[1].m_Height - 1) && (h < pForks[1].m_Height + 62) )
		{
			hEnd = pForks[1].m_Height + 63;
			return base / 5;
		}


		if (h == pForks[1].m_Height + 62)
		{
			hEnd = 64 + pForks[1].m_Height;

			Amount Reward = Emission.Frew + Emission.Fundgen + Emission.Fundinvest;

			return Reward;
		}


		assert(Emission.Drop2);
		Height n = 1 + (h - (pForks[1].m_Height + 63)) / Emission.Drop2;
		...
		hEnd = pForks[1].m_Height + 65 + n * Emission.Drop2;
