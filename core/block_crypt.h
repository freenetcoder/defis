...
namespace grimm
{
	...
	struct Rules
	{
		...
		struct {
			...
			Amount Frew = Coin * 13140000; // DEFIS Team allocated
			Amount Fundgen = Coin * 39420000; //Marketing & development DEFIS fund
			Amount Fundinvest = Coin * 34164000; //Allocated to DEFIS investors
			...
			Height Drop1	= 1440 * 365 * 4; // 4 years roughly. Each such a cycle there's a new
			Height Drop2 = 1440 * 2 * 365 * 4; // 4 years roughly for Defis. Each such a cycle there's a new
	...
	struct Block
	{
		...
		struct PoW
		{
			...
			static const uint32_t nNumIndices		= 1 << K;
			static const uint32_t nBitsPerIndex		= N / (K + 1) + 1;
			...
			static const uint32_t nSolutionBytes	= nSolutionBits >> 3;
